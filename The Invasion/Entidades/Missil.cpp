...
void Missil::setFase(Fases::Fase* pFaseAux){
    pFase = pFaseAux;
}

void Missil::AcertouJogador(Entidades::Personagens::Jogador* pJog){
    if(pJog)
        pJog->setVida(pJog->getVida() - dano);
    ApagarProjetil();
}

void Missil::ApagarProjetil(){
    if (pFase)
...
void Missil::CarregarSe(string atributos){
